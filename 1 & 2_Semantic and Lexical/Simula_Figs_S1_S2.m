...
for k = 1:N-1 % ciclo sul tempo
    ...
    % figura della semantica in tempo reale
    if round(k/20) == k/20
       linewidth= 1.5;
       fontsize= 11;
       stem(x_s(:,k+1),'linewidth',linewidth)      %da L a S + da S a S
       axis([0 Npop 0 1.2])
       set(gca,'fontsize',fontsize)
       xlabel('feature')
       titolo = ['step ' num2str(k+1)];
       title(titolo,'fontsize',fontsize)
       pause(0.5)
    end
...
plot(t(passoin:passofin),zp(98,passoin:passofin)/5,'-.k','LineWidth',2)
hold on
...
plot(t(passoin:passofin),x_l(21,passoin:passofin),'-.r','LineWidth',2)
hold on
