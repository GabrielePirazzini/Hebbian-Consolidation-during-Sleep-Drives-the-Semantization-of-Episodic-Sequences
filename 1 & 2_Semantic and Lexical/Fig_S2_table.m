...
scelta = 3; % 3 TABLE; 10 FISH DISH; 27 DAUGHTER, 35 CAT
...
figure
sgtitle('table','FontSize',16);
plot(t(passoin:passofin),x_s(1:2,passoin:passofin),'-.b','LineWidth',2) % mobile
hold on
plot(t(passoin:passofin),x_s([3 4 8],passoin:passofin),'-.k','LineWidth',2) % four legs-.
plot(t(passoin:passofin),x_s(5:7,passoin:passofin),'-.r','LineWidth',2) % distinctive
plot(t(passoin:passofin),x_s(9:149,passoin:passofin),':c','LineWidth',2) % distinctive
set(gca,'fontsize',18)
xlabel('time (s)')
ylabel('semantic network activity')
legend('it is in the house','it is built by the factory',...
'it has four legs', 'it is all wood','you eat on it','it has a plan', 'you put objects on it',...
'it has a table shape','other features','Location','southwest')
axes('position',[0.65 0.6 0.25 0.25])
plot(t(passoin:passofin),zp(5,passoin:passofin)/5,'-.r','LineWidth',2)
hold on
plot(t(passoin:passofin),zp([1:4 6:149],passoin:passofin)/5,'-.c','LineWidth',2)
legend('It has a plan','Other features')
set(gca,'fontsize',14)
xlabel('time (s)')
ylabel('cortical network activity')
axes('position',[0.65 0.2 0.25 0.25])
plot(t(passoin:passofin),x_l(1,passoin:passofin),'-.m','LineWidth',2)
hold on
plot(t(passoin:passofin),x_l([2:34],passoin:passofin),'-.c','LineWidth',2)
set(gca,'fontsize',14)
xlabel('time (s)')
ylabel('lexical network activity')
legend('Table','Other words')
