...
tau_d = 0.03; %0.038; %0.03
alpha_d = 1/tau_d;
beta_d = 6*alpha_d;  %4.8  %3*alpha_d
...
mpt=500; % input to pyramidal population
mft=0;
mct=500; % input to cholinergic population
...
for k=31:T-1 % ...from step 31 cause synaptic traing
    ...
    vec(:)=C(:,1).*ypc(:,k);
    vsc(:)=C(:,3).*ypc(:,k);
    vfc(:)=C(:,6).*ypc(:,k)-C(:,5).*ysc(:,k)-C(:,8).*yfc(:,k)+ylc(:,k);
    ...
    der_depl = alpha_d*(1 - Depletion(:,k)) - beta_d*xs(:,k).*Depletion(:,k); % si guarda solo l'attività pre-sunaptica
